...
import Imports;g=translate;o=True;d f=foldMap(\(x,y)->g x y f);q=circle;main=
 ...
 ;st t((w,s,m,z),a,d,b)=((v,p,l,q),a+t*e,e,u)where{p=(s`div`2)`xor`bool 0 180(
 odd s);e|a>30= -20|a< -30=20|o=d;(c,y)|Just b<-b=partition(n b)m|o=([], m);m1=
 map(\(x,y)->(x,y-t*10))y;u=b>>= \(x,y)->(x,y+t*20)<$guard(null c&&(y<30));(v,l)
 |w>0=(w-t,m1)|o=(0.35,(int2Float(s`mod`60)-30,30):m1);q|any(n(a,y0))y=0|o=z+length c}

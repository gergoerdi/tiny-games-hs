...
step t(w,s,a,d,m,b,z)=(w',s',a+t*d',d',m',b',z')
  where
    s'=(s`div`2) `xor` if odd s then 0xd008 else 0
    d'|a>30= -20|a< -30=20|True=d
    (c,nc)|Just b<-b=partition(n b)m|True = ([], m)
    m1=map(\(x,y)->(x,y-t*10))nc
    (w',m')|w>0=(w-t,m1)|True=(0.35,(fromIntegral(s`mod`80)-40, 30):m1)
    b' = b>>= \(x,y)->guard (null c&&(y<30))*>pure(x,y+t*20)
    z'|any(n(a,y0))nc = 0
      |True=z+length c
